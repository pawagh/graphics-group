...
const PublicationManager = memo(function PublicationManager({ onSave }: PublicationManagerProps) {
  ...
  const [pastResearchProjects, setPastResearchProjects] = useState<{ slug: string; title: string }[]>([]);
  ...
  useEffect(() => {
    fetch('/api/past-research')
      .then((r) => r.ok ? r.json() : [])
      .then((projects: { slug: string; title: string }[]) => setPastResearchProjects(projects))
      .catch(() => {});
  }, []);
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { abstract, summary, project, ...baseFormData } = formData;
      const body = isEditing
        ? { slug: editingPublication?.slug, ...baseFormData, ...(summary != null && { summary }), project: project || null }
        : { ...baseFormData, abstract, ...(summary != null && { summary }), project: project || null };
        ...
        <div className="bg-white p-6 rounded-lg shadow border">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {!isEditing && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Abstract (optional, for AI summary)
                </label>
                <textarea
                  rows={3}
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-carolina-blue focus:border-carolina-blue"
                  placeholder="Paste abstract for AI-generated key contributions summary"
                />
              </div>
            )}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Key contributions summary (optional)
              </label>
              <textarea
                rows={2}
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-carolina-blue focus:border-carolina-blue"
                placeholder="1-2 sentence summary of key contributions (auto-generated if AI is configured)"
              />
            </div>
            ...
            <div>
              ...
              >
                ...
                {pastResearchProjects.map((p) => (
                  <option key={p.slug} value={p.slug}>{p.title}</option>
                ))}
              </select>
              <p className="text-sm text-gray-500 mt-1">Link this paper to a past research project</p>
            </div>
